...
float ShadowCalculationCSM(vec3 fragPosWorld, vec3 N, vec3 L) {
    ...
    bias *= 1.0 / (float(layer) + 1.0); 
    vec2 texelSize = 1.0 / vec2(textureSize(shadowMapDir, 0));
    float closestDepth = texture(shadowMapDir, vec3(projCoords.xy, layer)).r;
    float distToCaster = max(0.0, currentDepth - closestDepth);
    
    const float PENUMBRA_SCALE_FACTOR = 1000.0;
    float radius = clamp(distToCaster * PENUMBRA_SCALE_FACTOR, 0.2, 30.0);
    
    // Sombra pura (não clareia)
    float shadow = 0.0; 
    for (int i = 0; i < 8; ++i) {
    ...
    // Desvanecimento por distância
    float maxDist = cascadePlaneDistances[cascadeCount-1];
    float fadeStart = 0.0;
    float fadeEnd = 20.0;
    float fadeFactor = (0.8 - smoothstep(fadeStart, fadeEnd, depthValue)) / 3;
...
float ShadowCalculation2D(vec4 fragPosLS, sampler2D shadowMap, vec3 N, vec3 L) {
    ...
    float distToCaster = max(0.0, currentDepth - closestDepth);
    const float PENUMBRA_SCALE_FACTOR = 80.0;
    float radius = clamp(distToCaster * PENUMBRA_SCALE_FACTOR, 0.5, 80.0); 
    float shadow = 0.0;
    for (int i = 0; i < 8; ++i) {
        vec2 offset = poisson8[i] * texelSize * radius;
